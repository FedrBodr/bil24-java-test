...
Статус: протокол одобрен 16.09.2026. Первоначальные измерения оригинала сохранены в `docs/results/baseline-initial/`; итоговые пары формируются при реализации.
...
| `repeatedMarkers` | Повторные `Ряд` и `Место`, сохраняющие особенности оригинала |
...
Подготовка измеряется дополнительно отдельным `TemplatePreparationBenchmark.compile`. Это разовая стоимость загрузки, её нельзя называть временем обработки запроса. Чтение файлов в эти CPU/память-измерения не входит; эффект дискового кэша операционной системы не приписывается оптимизации Java.
